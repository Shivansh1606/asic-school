...
const Messages = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      // Fetch President and Secretary from backend
      const [presidentRes, secretaryRes] = await Promise.all([
        managementAPI.getByPosition('president'),
        managementAPI.getByPosition('secretary')
      ]);

      const president = presidentRes.data[0];
      const secretary = secretaryRes.data[0];
      ...
      if (president) {
        messagesData.push({
          ...
          photo: getImageUrl(president.photo),
          message: president.message,
          fallback: `https://ui-avatars.com/api/?name=${encodeURIComponent(president.name)}&size=400&background=0C2E5C&color=FDB813&bold=true`
        });
      }
      ...
      if (secretary) {
        messagesData.push({
          ...
          photo: getImageUrl(secretary.photo),
          message: secretary.message,
          fallback: `https://ui-avatars.com/api/?name=${encodeURIComponent(secretary.name)}&size=400&background=0C2E5C&color=FDB813&bold=true`
        });
      }
      ...
      // Set default messages as fallback
      setMessages([
        {
          ...
          {messages.map((person, index) => (
            <div key={index} className="bg-white rounded-lg shadow-xl overflow-hidden hover:shadow-2xl transition">
              ...
              <div className="h-80 overflow-hidden bg-gray-200">
                <img
                  src={person.photo || person.fallback}
                  alt={person.name}
